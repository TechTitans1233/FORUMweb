...
document.getElementById("userLoginForm").addEventListener("submit", function (event) {
    ...
    .then(response => {
        if (!response.ok) {
            ...
            return response.json().then(err => { throw new Error(err.message || "Erro ao autenticar usuário: " + response.statusText); });
        }
        return response.json();
    })
    .then(data => {
        if (data.message === "Usuário autenticado com sucesso!" && data.token) {
            // **USAR OS DADOS DIRETAMENTE DA RESPOSTA DE LOGIN**
            localStorage.setItem("userEmail", data.email);
            localStorage.setItem("userName", data.name);
            localStorage.setItem("userId", data.uid); // Salvando o UID
            localStorage.setItem("userToken", data.token); // Salva o token

            alert("Login realizado com sucesso!");
            window.location.href = "../forum/forum.html";
        } else {
            throw new Error(data.message || "Token não recebido ou autenticação falhou.");
        }
    })
    .catch(error => {
        console.error(error);
